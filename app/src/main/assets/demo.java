...
public void onLoad(){
    ...
    // 第一个 String 代表文本内容
    api.sendTextMsg(api.createData("3340792396",false),"QScript 脚本发消息测试：表情：\u0014\u0003");
    // 发表情： 反斜杠u0014反斜杠u表情id

    // api.sendCardMsg(String 卡片代码, long 消息接受者, boolean 是否群聊)
    // 发送卡片需要完成高级验证，否则报错
    api.sendCardMsg(api.createData("818333976",true),"<?xml version='1.0' encoding='UTF-8' "+
    ...
    // api.sendPhotoMsg(data, String 图片路径)
    api.sendPhotoMsg(api.createData("818333976",true),"/sdcard/QQColor2/vip/fullBackground/chat/imgs_touch.jpeg");
...
public void onMsg(Object param){
    ...
    String[] atList = param.atList; // 被艾特的列表 String[]。注：老版本是 LinkedList
